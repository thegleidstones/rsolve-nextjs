...
import { fetchDataStatus, fetchData } from 'hooks/FetchData/fecthData'
...
function Complaint() {
  const { complaints, formComplaint, setFormComplaint, handleSubmit, handleInactivate, handleChange } = useComplaints();
  ...
  //   try {
  //     const response = await fetch("http://localhost:3344/complaints", {
  //       method: "POST",
  //       headers: {
  ...
  //   setFormComplaint((prevValues) => ({ ...prevValues, [name]: fieldValue }));
  // };
  ...
  useEffect(() => {
    const isDepartmentValid = formComplaint.departmentId.trim() !== '';
    const isFactValid = formComplaint.factId.trim() !== '';
    const isGrievanceDescriptionValid = formComplaint.complaintDescription.trim() !== '' && formComplaint.complaintDescription.length >= 20;
    const isWitnessDepartmentValid = formComplaint.witnessDepartmentId?.trim() !== '';
  ...
  }, [formComplaint.departmentId, formComplaint.factId, formComplaint.complaintDescription, formComplaint.witnessDepartmentId]);
  ...
  return (
    <Layout>
      ...
      <form onSubmit={handleSubmit}>
        <div className="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
          ...
          <div className="mt-4 pb-3">
            ...
            <Label>Prefere manter o anonimato ao enviar a denúncia?</Label>
            <div className="mt-2">
              <Label radio>
                <Input
                  type="radio"
                  name="isAnonymous"
                  value="true"
                  checked={formComplaint.isAnonymous === "true"}
              ...
              <Label className="ml-6" radio>
                <Input
                  ...
                  name="isAnonymous"
                  value="false"
                  checked={formComplaint.isAnonymous === "false"}
                  onChange={handleChange}
                />
            ...
            <div className="mt-2">
              <Label radio>
                <Input
                  ...
                  value="true"
                  checked={formComplaint.isOccuredInYourCompany === "true"}
                  onChange={handleChange}
                />
                <span className="ml-2">Sim</span>
              </Label>
              <Label className="ml-6" radio>
                <Input
                  ...
                  name="isOccuredInYourCompany"
                  value="false"
                  checked={formComplaint.isOccuredInYourCompany === "false"}
                  onChange={handleChange}
                />
                <span className="ml-2">Não</span>
              </Label>
          ...
          <Label className="mt-4">
            <span>Nos ajude a identificar o início do problema informando a empresa e o departamento envolvido nessa denúncia</span>
            <Label className="mt-4">
              <span>Empresa</span>
              <Select
                className="mt-1"
                name="companyId"
                disabled={formComplaint.isOccuredInYourCompany === "true"}
                value={formComplaint.companyId}
                onChange={handleChange}
              >
            ...
            <Label className="mt-4">
              ...
              <Select
                className="mt-1"
                name="departmentId"
                value={formComplaint.departmentId}
                onChange={handleChange}
              >
          ...
          <Label className="mt-4">
            <span>Essa denúncia é um fato isolado ou um fato recorrente?</span>
            <Select
              className="mt-1"
              name="factId"
              value={formComplaint.factId}
              onChange={handleChange}
            >
          ...
          <div className="mt-4 pb-3">
            {/* TODO: Check if this label is accessible, or fallback */}
            {/* <span className="text-sm text-gray-700 dark:text-gray-400">Account Type</span> */}
            <Label>Existe alguma testemunha para essa Denúncia?</Label>
            <div className="mt-2">
              <Label radio>
                <Input
                  type="radio"
                  name="isHaveAWitness"
                  value="true"
                  checked={formComplaint.isHaveAWitness === "true"}
                  onChange={handleChange}
                />
                <span className="ml-2">Sim</span>
              </Label>
              <Label className="ml-6" radio>
                <Input
                  ...
                  name="isHaveAWitness"
                  value="false"
                  checked={formComplaint.isHaveAWitness === "false"}
                  onChange={handleChange}
                />
                <span className="ml-2">Não</span>
              </Label>
          ...
          <Label>
            ...
            <Input
              className="mt-1"
              disabled={formComplaint.isHaveAWitness === "false"}
              name="witness"
              value={formComplaint.witness}
              onChange={handleChange}
          ...
          <Label>
            ...
            <Select
              ...
              value={formComplaint.witnessDepartmentId}
              onChange={handleChange}
            >
              <option value="">Selecione um departamento</option>
          ...
          <Label className="mt-4">
            ...
            <Textarea
              ...
              value={formComplaint.complaintDescription}
              onChange={handleChange}
      ...
      {isModalOpen && (
        <ModalResolve
          modalHeader="Registro de Denúncias"
          modalBody={protocol}
          onClose={closeModal}
          successMessage={true}
